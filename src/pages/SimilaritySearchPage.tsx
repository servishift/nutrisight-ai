...
export default function SimilaritySearchPage() {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    setError(null);
    setDemoMode(!import.meta.env.VITE_API_BASE_URL);
          ...
          <div className="lg:col-span-3">
            ...
            {result && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                ...
                {!demoMode && result.metadata && (
                  <Card className="border-success/20 bg-success/5">
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        <Info className="mt-0.5 h-5 w-5 shrink-0 text-success" />
                        <div className="space-y-1">
                          <p className="text-sm font-semibold text-foreground">Data Source: {result.metadata.dataSource}</p>
                          <p className="text-xs text-muted-foreground">
                            All products verified from official USDA FoodData Central database (1.9M+ products).
